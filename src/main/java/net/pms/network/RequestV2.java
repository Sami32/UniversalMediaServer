	...
	public ChannelFuture answer(
		...
		if ((method.equals("GET") || method.equals("HEAD")) && argument.startsWith("console/")) {
			...
			if (mediaInfo != null) {
				...
				output.headers().set("MediaInfo.sec: SEC_Duration=", timetotalinms); // Samsung renderers use it for subtitles (smi/caption)
			}
			...
			if (soapaction != null && soapaction.contains("ContentDirectory:1#GetSystemUpdateID")) {
				...
				if (files != null) {
					for (DLNAResource uf : files) {
						...
						if (
							uf.isCompatible(mediaRenderer) && (uf.getPlayer() == null
							...
							|| uf.getPlayer().isPlayerCompatible(mediaRenderer) && files.get(0).getParent() instanceof FileTranscodeVirtualFolder
						) {
