					...
					if (
						...
						for (DLNAMediaAudio audioTrack : media.getAudioTracksList()) {
							if (
								...
								!audioTrack.getCodecA().equals(FormatConfiguration.MP3) // up to 128 kbit/s only (WMVSPML_MP3)
							) {
								media.setContainer(FormatConfiguration.ASF);
								break;
							}
...
//		} else if (value.contains("bsac")) {
//			format = FormatConfiguration.AAC_BSAC;
			...
			if (value.equals("a_aac/mpeg2/main")) {
				...
				value.equals("aac") && FormatConfiguration.AVI.equals(media.getContainer())
			) {
			format = FormatConfiguration.AAC_LC;
		...
		} else if (value.equals("flac") || "19d".equals(value)) { // due to a flaw in MediaInfo code
